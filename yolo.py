...
class YOLO(object):
    ...
    # 检测图片
    def detection_image(self, image, crop = False, count = False, confidence=0.5, result_path=None):
        self.confidence = confidence

        # 如果没有提供结果文件路径，使用默认的res.txt
        if result_path is None:
            result_path = 'res.txt'
            
        # 清空结果文件
        with open(result_path, 'w', encoding='utf-8') as fb:
            fb.write('')
        ...
        try:
            ...
            for i, c in list(enumerate(top_label)):
                try:
                    ...
                    # 将检测结果写入指定的文件
                    with open(result_path, 'a', encoding='utf-8') as fb:
                    ...
                    draw.rectangle([tuple(text_origin), tuple(text_origin + np.array(label_size[2:]))], fill=self.colors[c])
    ...
    def detection_heatmap(self, image, heatmap_save_path):
        import cv2
